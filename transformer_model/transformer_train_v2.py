...
class ExpertActivationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        item = {
            ...
            'length': len(df),
            'layer_ids': layer_ids.detach().cpu()  # <=== Added
        }
        self.cache[idx] = item
        return item
...
def collate_fn(batch):
    ...
    layer_ids = [item['layer_ids'] for item in batch]  # <=== Added

    padded_features = pad_sequence(features, batch_first=True)
    padded_targets = pad_sequence(targets, batch_first=True)
    padded_layer_ids = pad_sequence(layer_ids, batch_first=True)  # <=== Added
    ...
    return {
        ...
        'lengths': lengths,
        'mask': mask,
        'layer_ids': padded_layer_ids  # <=== Added
    }
...
def validate(model, dataloader, device, writer, epoch):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            targets = batch['targets'].to(device)
            mask = batch['mask'].to(device)
            layer_ids = batch['layer_ids'].to(device)  # <=== Added
...
# (no changes to main() or training functions)
def main():
